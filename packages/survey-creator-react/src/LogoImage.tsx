    ...
    if (this.model.survey.locLogo.renderedHtml) {
      content = <>
        ...
        {this.model.allowEdit ? <div className="svc-image-item-value-controls">
          {attachKey2click(
            <span className="svc-image-item-value-controls__button svc-image-item-value-controls__choose-file">
              <SvgIcon size={24} iconName={"icon-file"} onClick={() => this.model.chooseFile(this.model)}></SvgIcon>
            </span>
          )}
          {attachKey2click(
            <span className="svc-image-item-value-controls__button svc-image-item-value-controls__remove">
              <SvgIcon size={24} iconName={"icon-delete"} onClick={() => this.model.remove(this.model)}></SvgIcon>
            </span>
          )}
        </div> : null}
      </>;
